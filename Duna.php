...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Status atualizado com sucesso!'); window.location.href = 'Duna.php?id={$livro_id}';</script>";
    } else {
        echo "<script>alert('Erro ao atualizar status.'); window.location.href = 'Duna.php?id={$livro_id}';</script>";
    }
    $stmt->close();
}
?>
...
<body id="background">
    ...
    <div class="container" id="acervo">
        <div class="card" id="livro">
            <img class="card-img-top" src="https://m.media-amazon.com/images/I/81zN7udGRUL.jpg" alt="Capa do livro">
            <div class="card-body">
                <a href="Duna-Livro.pdf" class="btn btn-primary">Ler</a>
            </div>
        </div>
        
        <div id="sinopse">
            <h1>Duna</h1>
            <h5>Nome do autor: Frank Herbert</h5>
            <h5>Data de lançamento: 1965</h5>
            <h5>Gênero: Aventura, Fantasia, Ficção cientifica, Romance, Suspense</h5>
            <h5>Sinopse do livro</h5>
            <p>Ambientado em um futuro distante, Duna é uma obra-prima da ficção científica que combina política, religião, ecologia e poder. A história se passa no planeta desértico Arrakis, conhecido como Duna, único lugar no universo onde se encontra a melange, ou especiaria, uma substância extremamente valiosa que prolonga a vida, expande a consciência e é essencial para a navegação espacial.</p>
            <p>O jovem Paul Atreides, herdeiro da Casa Atreides, é levado a Arrakis quando sua família assume o controle do planeta. No entanto, uma traição mortal tramada pelo rival, a Casa Harkonnen, força Paul e sua mãe, Lady Jessica, a fugir para o deserto, onde eles buscam refúgio entre os Fremen, o povo nativo do planeta. Lá, Paul descobre que ele pode ser o Kwisatz Haderach, um messias profetizado que tem o poder de alterar o destino da galáxia.</p>
            <p>Combinando uma narrativa rica e intrincada, Duna aborda temas como o impacto ambiental, luta pelo poder, lealdade e destino. Enquanto Paul enfrenta escolhas difíceis que moldarão o futuro de Arrakis e do universo, a obra desafia os leitores a refletirem sobre questões universais de liderança, moralidade e sobrevivência.</p>
