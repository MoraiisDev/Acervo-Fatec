...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Status atualizado com sucesso!'); window.location.href = 'O Sol é para Todos.php?id={$livro_id}';</script>";
    } else {
        echo "<script>alert('Erro ao atualizar status.'); window.location.href = 'O Sol é para Todos.php?id={$livro_id}';</script>";
    }
    $stmt->close();
}
?>
...
<body id="background">
    ...
    <div class="container" id="acervo">
        <div class="card" id="livro">
            <img class="card-img-top" src="https://m.media-amazon.com/images/I/81B84oHIFmL._UF894,1000_QL80_.jpg" alt="Capa do livro">
            <div class="card-body">
                <a href="O Sol e para Todos - Harper Lee.pdf" class="btn btn-primary">Ler</a>
            </div>
        </div>
        
        <div id="sinopse">
            <h1>O Sol é para Todos</h1>
            <h5>Nome do autor: Harper Lee</h5>
            <h5>Data de lançamento: 1960</h5>
            <h5>Gênero: Doméstica, Jurídica, Romance, Suspense</h5>
            <h5>Sinopse do livro</h5>
            <p>Ambientado na pequena cidade fictícia de Maycomb, no Alabama, durante a Grande Depressão, O Sol é para Todos é narrado pela jovem Scout Finch, que relembra os eventos que marcaram sua infância. A história explora temas de racismo, injustiça e moralidade através dos olhos inocentes de Scout e de seu irmão, Jem, enquanto eles enfrentam as realidades duras de uma sociedade preconceituosa.</p>
            <p>O pai de Scout, Atticus Finch, é um advogado íntegro que aceita defender Tom Robinson, um homem negro acusado injustamente de violentar uma mulher branca. A decisão de Atticus provoca tensões na cidade e coloca sua família sob os olhares de reprovação e hostilidade. Ao mesmo tempo, Scout e Jem começam a aprender sobre empatia, coragem e os complexos dilemas morais do mundo adulto.</p>
            <p>O Sol é para Todos é uma poderosa história sobre justiça, humanidade e compaixão, mostrando como uma única pessoa pode lutar contra a intolerância em meio a uma sociedade desigual. A obra é considerada um clássico da literatura americana e uma reflexão atemporal sobre os valores humanos.</p>
